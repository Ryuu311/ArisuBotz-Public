...
const client = {}
const jadibot = async (RyuuBotz, m, from) => {
  if (Object.keys(client).includes(from)) {
    return RyuuBotz.sendMessage(from, {
  ...
  const {
    state,
    saveCreds
  } = await useMultiFileAuthState(`./node_modules/Arisu-MD/database/rentbot/${m.sender.split("@")[0]}`)
  try {
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          let txt = `*[ 𝗝𝗔𝗗𝗜𝗕𝗢𝗧 - 𝗖𝗟𝗢𝗡𝗘 ]*\nᴋᴏᴅᴇ ᴘᴀɪʀɪɴɢ ᴍᴜ: ${code}\n\nᴍᴀsᴜᴋᴀɴ ᴋᴏᴅᴇ ᴋᴇ ᴘᴇʀᴀɴɢᴋᴀᴛ ᴛᴇʀᴛᴀᴜᴛ\nᴜɴᴛᴜᴋ ᴍᴇɴᴊᴀᴅɪ ʙᴏᴛ\n\nHai sayang, kamu mau jadi bot ya kayak aku😋. aku Arisu salam kenal🤩✨`
          await RyuuBotz.sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../RyuuID')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
      ...
      client[from].getFile = async (PATH, returnAsFilename) => {
        ...
        if (data && returnAsFilename && !filename)(filename = path.join(__dirname, './node_modules/Arisu-MD/search/' + new Date * 1 + '.' + type.ext), await fs.promises.writeFile(filename, data))
        return {
          res,
          filename,
          ...type,
      ...
      client[from].downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
        ...
        let trueFileName = attachExtension ? ('./node_modules/Arisu-MD/search/' + filename + '.' + type.ext) : './node_modules/Arisu-MD/search/' + filename
        // save to file
...
async function stopjadibot(RyuuBotz, m, from) {
  if (!Object.keys(client).includes(from)) {
    return RyuuBotz.sendMessage(from, {
  ...
  delete client[from]
  fs.unlinkSync(`./node_modules/Arisu-MD/database/rentbot/${from.split("@")[0]}`)
}

async function listjadibot(RyuuBotz, m) {
